...
EntryHashPlugins.prototype.apply = function(compiler) {
    ...
    compiler.plugin('done', function(stats) {
        let toReplace = _this.toReplace;
        const hash = stats.hash;
        const hashLength = checkHash(targetFilename);
        const keyName = checkName(targetFilename);
        if (keyName) {
            targetFilename = targetFilename.replace(/\[name\]/i, _this.toReplace.match(/\[name\]=??(\w+)/)[1]);
            toReplace = toReplace.replace(/\?.+/, '');
        }
        if (undefined === hashLength || !_this.toReplace) return;
